...
document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll(".qodef-m-icon-link").forEach(function (box) {
    let bgImage = box.getAttribute("data-bg");
    if (bgImage) {
      box.style.setProperty("--bg-image", `url(${bgImage})`);
      box.querySelector("::before").style.backgroundImage = `url(${bgImage})`;
    }
  });
});